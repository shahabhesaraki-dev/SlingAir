...
const Plane = ({ flightNumber }) => {
  const { setSelectedSeat, seating, setSeating, selectedSeat } =
    useContext(SeatContex);
          ...
          seating.map((seat) => (
            <SeatWrapper key={`seat-${seat._id}`}>
              <label>
                {seat.isAvailable ? (
                  <>
                    <Seat type="checkbox" name={seat._id} />
                    <Available
                      onClick={() => {
